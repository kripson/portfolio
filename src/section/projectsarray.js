export const projectsarray = [
    {
        id: 1,
        title: 'One Wish Cleaning',
        imageUrl: require("../assets/One wish cleaning no bg (2).webp"),
        mobileLayout: require("../assets/onewishcleaningmobile.webp"),
        laptopLayout: require("../assets/onewishcleaninglaptop.png"),
        description: ["Complete Job management and rostering solution developed for One Wish Cleaning Pty Ltd. The project's objective was to create a solution that caters to client's exact business needs. I was responsible for designing, developing, testing and deploying the application.",
            "The platform is built on MERN stack. The front-end is powered by React and the back-end uses serverless framework and utilizes AWS lambda, S3 bucket and Event bridge for scheduled jobs."],
        websiteUrl: 'https://dash.onewishcleaning.com.au',
        ready: true,
        year: '2024',
        height: '30vh',
        tools: ['NextJs'],
        helperImage: require("../assets/onewishcleaninghelper.png"),
        styles: {
            backgroundColor: '#a5edff',
            color: 'white',
            detailColor: 'black',
            filter: `drop-shadow(-20px 40px 10px rgba(0,0,0,0.25))`
        },
        tags: [
            'React',
            'AWS',
            'Serverless',
            'MongoDB'
        ],
        heading: 'Projects'
    },
    {
        id: 2,
        title: 'Smile your way to happy',
        imageUrl: require("../assets/Smile your way to happy.webp"),
        mobileLayout: require("../assets/smileyourwaytohappymobile.webp"),
        laptopLayout: require("../assets/smileyourwaytohappylaptop.webp"),
        description: ["Project developed for Bernie Hayne. This project serves as a landing page for Bernie's first children book called smileyourwaytohappy. I was responsible for designing, developing, testing and deploying the application.",
            "The front-end was created using NextJs and styled using custom css."],
        websiteUrl: 'https://smileyourwaytohappy.com',
        ready: true,
        height: '35vh',
        year: '2022-23',
        tools: ['NextJs'],
        helperImage: require("../assets/smileyourwaytohappyhelper.webp"),
        styles: {
            backgroundColor: '#ffb2db',
            color: 'white',
            detailColor: 'black',
            filter: `drop-shadow(-20px 40px 10px rgba(0,0,0,0.25))`
        },
        tags: [
            'NextJs',
            'SCSS',
            'Web Design',
            'GSAP'
        ]
    },
    {
        id: 3,
        title: 'Learn your way to happy',
        imageUrl: require("../assets/Learn your way to happy.webp"),
        mobileLayout: require("../assets/learnyourwaytohappymobile.webp"),
        laptopLayout: require("../assets/learnyouwaytohappylaptop.webp"),
        description: ["Project developed for Bernie Hayne. This project serves as a landing page for Bernie's second children book called learnyourwaytohappy. I was responsible for designing, developing, testing and deploying the application.",
            "The front-end was created using NextJs and styled using custom css."],
        websiteUrl: 'https://learnyourwaytohappy.com',
        ready: true,
        height: '35vh',
        year: '2023-24',
        tools: ['NextJs'],
        helperImage: require("../assets/learnyourwaytohappymobile.webp"),
        styles: {
            backgroundColor: '#c8afff',
            color: 'white',
            filter: `drop-shadow(-20px 40px 10px rgba(0,0,0,0.25))`
        },
        tags: [
            'NextJs',
            'SCSS',
            'Web Design',
            'GSAP'
        ]
    },
    {
        id: 4,
        title: 'Kzen8',
        imageUrl: require("../assets/Kzen8-project.png"),
        mobileLayout: require("../assets/kzen8mobile.png"),
        laptopLayout: require("../assets/kzen8laptop.png"),
        description: ["Project developed for Kzen8 Pty Ltd. This is an ongoing project serves as a landing page for a no-code business software platform provided by kzen8 and is part of a much bigger marketing campaign. I was responsible for designing, developing, testing and deploying the application.",
            "The front-end was created using React and styled using custom css."],
        websiteUrl: 'https://kzen8.com',
        ready: true,
        year: '2020-24',
        height: '45vh',
        tools: ['React'],
        helperImage: require("../assets/kzen8helper.png"),
        styles: {
            backgroundColor: '#40469d',
            color: '#DFD3C3'
        }
    },
    {
        id: 5,
        title: 'Little Hideout Cafe',
        imageUrl: require("../assets/littlehideoutcafe.webp"),
        mobileLayout: require("../assets/littlehideoutmobile.png"),
        laptopLayout: require("../assets/littlehideoutlaptop.png"),
        description: ["Project developed for Little Hideout Cafe. The project's objective was to give the outdated website a new look and feel and also develop a custom booking feature. I was responsible for designing, developing, testing and deploying the application.",
            " The front-end was created using React and styled using custom css. The back-end server was developed using Express Js which is connected to a MongoDB database."],
        websiteUrl: 'https://littlehideoutcafe.com.au',
        ready: true,
        year: '2021',
        height: '40vh',
        tools: ['react', 'node'],
        helperImage: require("../assets/littlehideouthelper.webp"),
        styles: {
            backgroundColor: 'white',
            color: 'black'
        },
        tags: [
            'React',
            'SCSS',
            'Express',
            'MongoDB'
        ]
    },

];
